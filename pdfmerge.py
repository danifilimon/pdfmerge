...
def open_file(files):
    filepath = askopenfilenames(filetypes=[("PDF Files", "*.pdf"), ("All Files", "*.*")])
    for file in filepath:
        if not (file and Path(file).exists()):
            return
        files.append(file)
...
btn_open = tk.Button(fr_bg1, text="Open file(s)", command=lambda: open_file(filelist))
...
btn_merge = tk.Button(
    fr_bg2, text="Merge PDF", state="disabled", command=lambda: merge_pdfs(filelist)
)
